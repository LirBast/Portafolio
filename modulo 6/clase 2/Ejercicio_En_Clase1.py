...
print(f'Desviacion estandar de las exactitudes en los folds: {std_accuracies_kfold:.2f}')

kfold_scores=cross_val_score(DecisionTreeClassifier(random_state=42),X,y, cv=kf,scoring='accuracy')
print(f'Presicion por subconjunto(KFold_cross_val_score): {kfold_scores}')
print(f'Media de la presicion por subconjunto(KFold_cross_val_score): {kfold_scores.mean():.2f}')
print(f'Desviacion estandar de la presicion por subconjunto(KFold_cross_val_score): {kfold_scores.std():.2f}')

loo = LeaveOneOut()
loo_scores=cross_val_score(DecisionTreeClassifier(random_state=42),X,y,cv = loo,scoring='accuracy')
print(f'La exactitud loo_scores: {loo_scores}')
print(f'Media de la exactitud loo_scores: {loo_scores.mean():.2f}')
print(f'Desviacion estandar de la exactitud loo_scores: {loo_scores.std():.2f}')