...
print("\n" + "="*80 + "\n")
print('Conteo de etiqueta sentimiento')
...
print('\nResumen de la arquitectura del modelo GRU\n')
model.build(input_shape=(None, max_length))
print(model.summary())

history=model.fit(X_train_pad, y_train, epochs=5, batch_size=32,validation_data=(X_test_pad, y_test))

model.save('GRU_review_model.h5')

loss, accuracy = model.evaluate(X_test_pad, y_test, verbose=0)

print('\nResultado de la evaluacion del conjunto de prueba GRU\n')
print(f'Pérdida: {loss:.4f}')
print(f'Precisión: {accuracy:.4f}')

y_predicho_proba=model.predict(X_test_pad)
y_predicho=(y_predicho_proba>0.5).astype('int64')
print('\nReporte de clasificacion\n')
print(classification_report(y_test, y_predicho,target_names=['negativo','positivo']))
